...
define('debug', default=False,
        type=bool, help="Run in debug mode")
define('port', default=8088,
        type=int, help="Server port")
define('allowed_hosts', default="localhost:8088",
        multiple=True,
        help="Allowed hosts for cross domain connections.")

class SprintHandler(WebSocketHandler):
    """Handles real-time updates on board"""

    def check_origin(self, origin):
        allowed = super().check_origin(origin=origin)
...
class ScrumApplication(Application):
    ...
    def broadcast(self, message, channel=None, sender=None):
        if channel is None:
            for c in self.subscription.keys():
...
if __name__ == '__main__':
    ...
    logging.info('Starting websockets server on http://localhost:{}'.format(options.port))
